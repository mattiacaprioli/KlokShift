...
- **Due inviti, due meccaniche diverse, apposta.** Il professionista si registra da sé e l'account resta suo (è il suo profilo di carriera fra più aziende). Il collaboratore no: l'account glielo crea la Edge Function `invite-staff` con `generateLink({ type: 'invite' })` e `role: 'manager'` nei metadati, e a lui resta solo aprire il link e scegliere una password (atterraggio su `web/`, `#/imposta-password`). Un collaboratore è un posto **dentro** l'azienda del titolare — non può possedere sedi — quindi pre-crearglielo non gli toglie niente. ⚠️ Il prezzo è che l'indirizzo resta occupato finché l'invito non viene aperto o ripulito: vedi `claim_venue_access_cancel` + `inert_invite_user` (20260917110000), chiamate dal ramo `team_cancel`.
